...
class ContactCollection implements IAddressBook, IProperties, IMultiGet, ISyncCollection {
	...
	private RemoteContactsService $_store;
	private ?LocalContactsService $_localContactService = null;
	private ContactCollectionObject $_collection;
	protected array $_entitiesCache = [];

	/**
	 * Collection constructor.
	 */
	public function __construct(string $user, RemoteContactsService $store, ContactCollectionObject $data) {
	...
	public function getProperties($properties): array {
		...
		return [
			...
			'{http://owncloud.org/ns}enabled' => (string)$this->_collection->Visibility,
		];
	}
	...
	public function propPatch(PropPatch $propPatch): void {
		...
		if (count($mutations) > 0) {
			...
			if (isset($mutations['{DAV:}displayname'])) {
				$this->_collection->Label = $mutations['{DAV:}displayname'];
	...
	public function getMultipleChildren(array $ids): array {
		...
		if (count($ids) === count(array_intersect_key($this->_entitiesCache, array_flip($ids)))) {
			...
			foreach ($this->_store->entityFetchMultiple($this->_collection->Id, $ids) as $entity) {
	...
	public function getChild($id): ContactEntity|false {
		...
		if (isset($this->_entitiesCache[$id])) {
			...
			$entity = $this->_store->entityFetch($this->_collection->Id, $id);
		...
		return $entity !== null ? new ContactEntity($this, $entity) : false;
	}
	 ...
	 *
	 * @param string $id existing entity object
	...
	public function fromContactObject(ContactObject $so): string {
		if ($this->_localContactService === null) {
			$this->_localContactService = new LocalContactsService();
		}
		$do = $this->_localContactService->fromContactObject($so);
		return $do->serialize();
	}
	...
	public function toContactObject(VCard|string $so): ContactObject {
		...
		$do = $this->_localContactService->toContactObject($so);
		return $do;
	}
