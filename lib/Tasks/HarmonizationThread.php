...
try {
	...
	if (php_sapi_name() == 'cli') {
		...
		$logger->info("Harmonization thread executed from console", ['app' => 'integration_jmapc']);
		echo "Harmonization thread executed from console" . PHP_EOL;

		// evaluate if required function exists
        if (!function_exists('posix_getuid')) {
			$logger->info("Harmonization thread failed missing required posix extensions - see https://www.php.net/manual/en/book.posix.php", ['app' => 'integration_jmapc']);
	...
	elseif ($_SERVER['REMOTE_ADDR'] != '127.0.0.1' && $_SERVER['REMOTE_ADDR'] != '::1') {
		$logger->info("Harmonization thread failed can only be executed from the console or localhost", ['app' => 'integration_jmapc']);
	...
	if (!(bool) \OC::$server->getConfig()->getSystemValue('installed', false)) {
		$logger->info("Harmonization thread failed system installed status is false", ['app' => 'integration_jmapc']);
	...
	if ((bool) \OC::$server->getSystemConfig()->getValue('maintenance', false)) {
		$logger->info("Harmonization thread failed system maintenance mode is on", ['app' => 'integration_jmapc']);
	...
	if (getmypid() != $tid && $HarmonizationThreadService->isActive($uid, $tid)) {
		$logger->info("Harmonization thread failed another thread is already running for $uid", ['app' => 'integration_jmapc']);
	...
	if (!$ConfigurationService->isAccountConnected($uid)) {
		$logger->info("Harmonization thread failed user $uid does not have a connected account", ['app' => 'integration_jmapc']);
	...
	$logger->info("Harmonization thread started for $uid", ['app' => 'integration_jmapc']);
	...
	$logger->info("Harmonization thread ended for $uid", ['app' => 'integration_jmapc']);
	...
	$logger->info('Harmonization thread ended unexpectedly', ['app' => 'integration_jmapc']);
	echo $ex . PHP_EOL;
	exit(1);
} catch (Error $ex) {
	$logger->logException($ex, ['app' => 'integration_jmapc']);
